...
```json
[
  { "slug": "quotes", "label": "Quotes", "description": "...", "route": "/quotes", "icon": "Calculator" },
  { "slug": "garments", "label": "Garments", "description": "...", "route": "/garments", "icon": "Shirt" }
]
```
...
Zod schema pattern:
```typescript
const pipelineFieldEntry = z.object({
  ...
  .refine((f) => !f.updatable || f.flag !== undefined, { message: "Updatable fields must have a flag" })
  .refine((f) => f.jsonType !== "boolean" || !f.flag || f.negateFlag !== undefined, { message: "Boolean fields with flag must have negateFlag" });
