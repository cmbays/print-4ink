...
function NoteItem({ note, onTogglePin }: { note: Note; onTogglePin: (id: string) => void }) {
  return (
    <div
      className={cn(
        'rounded-md border p-3 space-y-2',
        note.isPinned ? 'bg-action/5 border-action/20' : 'bg-background border-border'
      )}
    >
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          {note.channel && (
            <Badge variant="ghost" className={cn(CHANNEL_COLORS[note.channel], 'text-xs')}>
