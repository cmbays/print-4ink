# Screen Printing Price Matrix — Industry Best Practices Research

**Research Date:** February 10, 2026
**Researcher:** industry-researcher
**Task:** Comprehensive industry analysis of screen printing pricing structures
...
- Separate contract vs. retail pricing structures
- Rush order upcharge calculation
- Specialty ink surcharges

---

## 1. Standard Pricing Dimensions

### 1.1 Quantity Breaks (Primary Driver)
...
- Mirrors **wholesale garment ordering** patterns (sold by the dozen)
- Aligns with **press efficiency** (most shops optimize around 24-72 piece runs)
- **Setup cost amortization** — larger orders dilute per-piece setup costs

**Practitioner Insights (T-Shirt Forums):**

- Average minimums: 12, 18, 24, 36, 48, 64, 72 (most efficient shops)
- Price drops dramatically: "$8 per shirt at 12 qty, $4 at 144 qty, $3 at 288 qty"
...
### 1.2 Color Count (Second Most Critical)
...
### 3.1 Per-Piece Tiered Pricing (Most Common)

**Structure:**

```
Base Formula = (Garment Cost × Markup) + (Setup Fees ÷ Quantity) + Print Fees

Print Fees = (Colors × Locations × Per-Hit Rate)
```
...
Total Costs = Materials + Labor + Overhead + Setup
```
...
### 5.1 How Volume Discounts Work

**Price Inversion:**

Screen printing exhibits **inverse pricing** — per-piece cost **decreases** as quantity **increases**, because:

1. **Fixed setup costs** amortize over more units
2. **Press efficiency** improves (longer runs = less downtime)
3. **Garment wholesale pricing** offers bulk discounts
...
- 72 shirts: **$5.00 each** (-16.7%)
- 144 shirts: **$4.00 each** (-20%)
...
### 5.2 Quantity Break Automation Features

**Table Stakes for Price Matrix Software:**

1. **Auto-calculate breaks** based on setup cost recovery
2. **Visual price curves** showing savings per tier
3. **"Best value" highlighting** (e.g., "Save $50 by ordering 48 instead of 36")
4. **Custom tier configuration** (shops set their own break points)
5. **Margin protection** (never price below minimum margin)

**Advanced Features:**

- **Dynamic breaks** based on current garment inventory
- **Rush order tier adjustments** (fewer breaks, higher minimums)
- **Customer-specific volume discounts** (contract customers get steeper curves)

---

## 6. Industry-Specific Considerations

### 6.1 Specialty Inks & Upcharges
...
### 6.3 Minimum Order Quantities (MOQ)

**Industry Standard MOQs:**

- **24 pieces** (most common)
- **12 pieces** (budget shops, higher per-piece price)
- **36 pieces** (premium shops, better margins)
...
**Total Setup Time:** 1–2 hours per job, regardless of quantity

**MOQ Calculation:**

```
MOQ = (Desired Hourly Rate × Setup Hours) ÷ (Target Profit Per Piece)

Example: ($50/hour × 1.5 hours) ÷ $5 profit = 15 pieces minimum
```

**Small Order Alternatives:**

- **DTG (Direct-to-Garment):** No setup fees, economical for 1–12 pieces
- **Heat transfers:** Pre-made transfers, lower MOQ (1–6 pieces)
...
## 7. Table Stakes Features (Must-Have for Any Price Matrix Software)

### 7.1 Core Functionality

**Non-Negotiable Features:**

1. **Multi-Dimensional Pricing Engine**
   - Quantity × Colors × Locations × Garment Type
   - Real-time calculation as user adjusts inputs
   - Support for 3+ simultaneous print locations

2. **Quantity Break Configuration**
   - Define custom break points (default: 12, 24, 48, 72, 144+)
   - Auto-calculate pricing for each tier
   - Visual price curve/table display

3. **Setup Fee Management**
   - Per-screen fees (separate from per-piece pricing)
   - Reorder discount logic (waive fees within X months)
   - Bulk order setup fee waiver (72+ pieces)

4. **Garment Library**
   - Garment types with wholesale costs
   - Configurable markup percentages by type
   - Brand/style/color variants

5. **Print Location Manager**
   - Standard locations (front, back, sleeves, pocket)
   - Per-location upcharges
   - Multi-location pricing rules

6. **Pricing Mode Toggle**
   - Retail pricing (high margin, full setup fees)
   - Contract pricing (low margin, waived fees)

7. **Rush Order Upcharges**
   - Percentage-based turnaround upcharges
   - Turnaround time selector (standard, rush, emergency)

8. **Specialty Ink Upcharges**
   - Water-based, discharge, metallic, etc.
   - Per-print upcharge configuration
...
1. **Real-Time Price Updates**
   - Price updates as user changes any input
   - No "Calculate" button needed

2. **Quantity Break Comparison Table**
   - Show pricing for all break tiers simultaneously
   - Highlight "best value" tier
   - Display total savings vs. next-lower tier

3. **Price Breakdown Transparency**
   - Show: Garment cost + Setup + Print fees = Total
   - Collapsible detail view for full breakdown

4. **Quote Generation**
   - PDF quote with line-item breakdown
   - Email quote directly to customer
   - Save/retrieve quotes

5. **Margin Visibility (Internal)**
   - Show cost vs. price for shop owner
...
## 8. How Pricing Data Is Organized & Maintained

### 8.1 Common Data Structures

**Pricing Matrix Formats:**

1. **Spreadsheet-Based (Excel/Google Sheets)**
   - Most common among small shops
   - Rows = Quantity breaks, Columns = Color counts
   - Separate sheets per garment type
   - Formula-driven, easy to adjust

2. **Database-Driven (Software)**
   - Separate tables for: Garments, Setup Fees, Print Fees, Breaks
   - Relational model allows complex rules
   - Version control for price changes

**Example Matrix Structure:**

```
Garment: Gildan 5000 (White Cotton Tee)
Base Cost: $3.00
Markup: 200%

| Quantity | 1-Color | 2-Color | 3-Color | 4-Color |
|----------|---------|---------|---------|---------|
| 12-23    | $9.50   | $10.50  | $11.50  | $12.50  |
| 24-47    | $8.00   | $9.00   | $10.00  | $11.00  |
| 48-71    | $7.00   | $8.00   | $9.00   | $10.00  |
| 72-143   | $6.00   | $7.00   | $8.00   | $9.00   |
| 144+     | $5.00   | $6.00   | $7.00   | $8.00   |
```

### 8.2 Maintenance Best Practices

**Pricing Updates (From Practitioner Forums):**

1. **Annual Price Review**
   - Review garment wholesale costs (suppliers change prices)
   - Adjust for inflation (ink, screens, utilities)
   - Benchmark against local competitors

2. **Garment Cost Tracking**
   - Update costs when placing wholesale orders
   - Account for freight/shipping in true cost
   - Track preferred brands/styles separately

3. **Customer-Specific Pricing**
   - Contract customers get custom matrices
   - Volume commitments = steeper discounts
   - Store customer pricing history for consistency

4. **Version Control**
   - Date pricing changes
   - Grandfather existing quotes (honor old prices for X days)
   - Archive old matrices for reference

**Software Integration:**

- **Garment wholesale catalogs** (API integrations with SSActivewear, SanMar)
...
## 9. Competitive Software Solutions
...
**What Makes a Price Matrix Software Successful:**

1. **Accuracy > Speed**
   - Shops prioritize correct pricing over fast quoting
   - Underpricing = lost profit, Overpricing = lost jobs
   - "Keep very detailed notes on everything" (practitioner advice)

2. **Flexibility Matters**
   - Every shop has unique cost structures
   - No "one size fits all" pricing
   - Custom breaks, custom markups, custom rules

3. **Transparency Builds Trust**
   - Show breakdown (garment + setup + print = total)
   - Explain why setup fees exist
   - Educate customers on quantity break savings

4. **Margin Protection**
   - Never let users quote below cost
   - Flag low-margin jobs automatically
   - Track actual costs (not just estimates)

### 10.2 Common Pitfalls (From Forums)

**What Shop Owners Struggle With:**

1. **Underestimating True Costs**
   - Forgetting overhead (rent, utilities, waste)
   - Not tracking ink usage accurately
   - Ignoring labor for setup/cleanup

2. **Inconsistent Pricing**
   - Different prices for same job (customer confusion)
   - No version control on pricing changes
   - Verbal quotes vs. written quotes mismatch

3. **Over-Complicating Matrices**
   - Too many variables = decision paralysis
   - Start simple (quantity + colors only), add complexity later

4. **Ignoring Competitors**
   - Pricing in a vacuum leads to losing bids
...
### 10.3 P0/P1/P2 Feature Recommendations

**P0 (Must-Have — MVP Blocking):**

1. Quantity break calculator (12, 24, 48, 72, 144+ tiers)
2. Multi-color pricing (1–6 colors)
3. Print location upcharges (front, back, sleeves)
4. Garment type selection with cost lookup
5. Setup fee calculation (per-screen, with bulk waiver)
6. Real-time price updates
7. Quote breakdown display (garment + setup + print)
8. PDF quote export

**P1 (High-Value — Complete Product):**

9. Contract vs. Retail pricing toggle
10. Rush order upcharge calculator
11. Specialty ink upcharges (discharge, water-based, etc.)
12. Reorder discount logic (waive setup fees)
13. Margin visibility (internal dashboard)
14. Quote save/retrieve
15. Customer-specific pricing overrides

**P2 (Differentiators — Competitive Advantage):**

16. Capacity-based pricing (adjust for shop load)
17. Garment API integration (SSActivewear, SanMar)
18. "Best value" quantity highlighting
19. Competitor price benchmarking
20. Historical quote analytics
21. Dynamic break suggestions (optimize for margin + win rate)
...
Screen printing price matrix software must balance **simplicity** (easy for customers to understand) with **complexity** (accurate cost modeling for shop profitability). The industry has converged on a **tiered quantity break model** with **per-color and per-location upcharges**, anchored by **setup fees** that amortize over larger orders.

**Key Takeaway:** Successful pricing matrices are **dynamic** (adjust for volume, garment type, complexity) yet **transparent** (customers understand why prices change). The software should **protect margins** while **staying competitive**, and must be **flexible** enough to accommodate each shop's unique cost structure.
