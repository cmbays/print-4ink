...
export function projectJobToCard(job: Job): JobCard { /* joins 5 entities */ }
```
...
import 'server-only';
const provider = process.env.DATA_PROVIDER === 'supabase'
...
async function createInvoice(input: NewInvoice): Promise<Result<Invoice,
  'CUSTOMER_NOT_FOUND' | 'DUPLICATE_INVOICE_NUMBER' | 'VALIDATION_FAILED'>>
```
