...
export function isValidStatusTransition(current: InvoiceStatus, target: InvoiceStatus): boolean {
...
export function calculateDueDate(createdAt: string, paymentTerms: string): string {
...
export function convertQuoteToInvoiceLineItems(quote: Quote): Omit<InvoiceLineItem, 'id'>[] {
  const items: Omit<InvoiceLineItem, 'id'>[] = []

  for (const lineItem of quote.lineItems) {
    const totalQty = Object.values(lineItem.sizes).reduce((sum, qty) => sum + qty, 0)
