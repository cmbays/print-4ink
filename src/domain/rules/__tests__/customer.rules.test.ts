...
describe('propagateAddition (N22)', () => {
  ...
  it('skips inherit-mode brands (they get it via resolution)', () => {
    const bellaCanvas = brandPreferences.find((b) => b.brandName === 'Bella+Canvas')!
  ...
  it('is idempotent — does not duplicate on repeat calls', () => {
    ...
    const countAfterFirst = gildan.favoriteColorIds.filter((id) => id === 'clr-forest-green').length
    propagateAddition(
      'global',
      'clr-forest-green',
      brandPreferences,
  ...
  it('warns on brand-level propagation (Phase 1 stub)', () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Phase 1 stub'))
    warnSpy.mockRestore()
  })
})
