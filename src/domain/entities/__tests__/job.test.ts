...
describe('laneEnum', () => {
  it.each(['ready', 'in_progress', 'review', 'blocked', 'done'])("accepts '%s'", (lane) => {
...
describe('riskLevelEnum', () => {
  it.each(['on_track', 'getting_tight', 'at_risk'])("accepts '%s'", (risk) => {
...
describe('jobTaskSchema', () => {
  ...
  it('validates a task with optional detail', () => {
    expect(() => jobTaskSchema.parse({ ...validTask, detail: 'Stitch file created' })).not.toThrow()
  })
  ...
  it('rejects negative sortOrder', () => {
    expect(() => jobTaskSchema.parse({ ...validTask, sortOrder: -1 })).toThrow()
  })
})
...
describe('jobNoteSchema', () => {
  ...
  it('rejects empty content', () => {
    expect(() => jobNoteSchema.parse({ ...validNote, content: '' })).toThrow()
  })

  it('rejects empty author', () => {
    expect(() => jobNoteSchema.parse({ ...validNote, author: '' })).toThrow()
  })
})
...
describe('jobComplexitySchema', () => {
  ...
  it('rejects zero garmentVariety', () => {
    expect(() => jobComplexitySchema.parse({ locationCount: 1, garmentVariety: 0 })).toThrow()
  })

  it('rejects negative locationCount', () => {
    expect(() => jobComplexitySchema.parse({ locationCount: -1, garmentVariety: 1 })).toThrow()
  })
})
...
describe('jobSchema', () => {
  const validJob = {
    ...
    printLocations: [{ position: 'Front Center', colorCount: 3, artworkApproved: true }],
    ...
    tasks: [
      ...
      {
        id: '1a240002-0001-4001-8001-000000001025',
  ...
  it('rejects invalid job number format', () => {
    expect(() => jobSchema.parse({ ...validJob, jobNumber: '1024' })).toThrow()
    expect(() => jobSchema.parse({ ...validJob, jobNumber: 'J-12' })).toThrow()
  })

  it('rejects invalid lane value', () => {
    expect(() => jobSchema.parse({ ...validJob, lane: 'cancelled' })).toThrow()
  })

  it('rejects invalid service type', () => {
    expect(() => jobSchema.parse({ ...validJob, serviceType: 'sublimation' })).toThrow()
  })

  it('rejects invalid date format', () => {
    expect(() => jobSchema.parse({ ...validJob, dueDate: 'Feb 14, 2026' })).toThrow()
  })

  it('rejects zero quantity', () => {
    expect(() => jobSchema.parse({ ...validJob, quantity: 0 })).toThrow()
  })
