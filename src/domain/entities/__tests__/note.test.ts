...
describe('noteChannelEnum', () => {
  it.each(['phone', 'email', 'text', 'social', 'in-person'])("accepts '%s'", (channel) => {
...
describe('noteSchema', () => {
  ...
  it('rejects invalid datetime', () => {
    expect(() => noteSchema.parse({ ...validNote, createdAt: 'not-a-date' })).toThrow()
  })

  it('rejects invalid entityType', () => {
    expect(() => noteSchema.parse({ ...validNote, entityType: 'order' })).toThrow()
  })

  it('rejects invalid entityId UUID', () => {
    expect(() => noteSchema.parse({ ...validNote, entityId: 'not-a-uuid' })).toThrow()
  })
})
