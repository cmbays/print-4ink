...
describe('colorSchema', () => {
  ...
  it('rejects invalid hex format — missing hash', () => {
    expect(() => colorSchema.parse({ ...validColor, hex: '000000' })).toThrow()
  })

  it('rejects invalid hex format — too short', () => {
    expect(() => colorSchema.parse({ ...validColor, hex: '#000' })).toThrow()
  })

  it('rejects invalid hex format — too long', () => {
    expect(() => colorSchema.parse({ ...validColor, hex: '#0000000' })).toThrow()
  })

  it('rejects invalid hex format — non-hex chars', () => {
    expect(() => colorSchema.parse({ ...validColor, hex: '#ZZZZZZ' })).toThrow()
  })

  it('rejects invalid swatchTextColor format', () => {
    expect(() => colorSchema.parse({ ...validColor, swatchTextColor: 'white' })).toThrow()
  })

  it('rejects invalid hex2 format', () => {
    expect(() => colorSchema.parse({ ...validColor, hex2: 'not-a-hex' })).toThrow()
  })

  it('rejects empty name', () => {
    expect(() => colorSchema.parse({ ...validColor, name: '' })).toThrow()
  })

  it('rejects empty family', () => {
    expect(() => colorSchema.parse({ ...validColor, family: '' })).toThrow()
  })
