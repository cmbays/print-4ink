...
describe('severityEnum', () => {
  it.each(['critical', 'major', 'warning', 'info'])("accepts '%s'", (val) => {
...
describe('ruleScopeEnum', () => {
  it.each(['local', 'cross-file', 'architectural'])("accepts '%s'", (val) => {
...
describe('reviewRuleSchema', () => {
  ...
  it('accepts optional goodExample', () => {
    ...
    expect(result.goodExample).toBe('type Foo = z.infer<typeof fooSchema>;')
  })

  it('rejects description shorter than 10 chars', () => {
    expect(() => reviewRuleSchema.parse({ ...validRule, description: 'Too short' })).toThrow()
  })

  it('rejects detection shorter than 10 chars', () => {
    expect(() => reviewRuleSchema.parse({ ...validRule, detection: 'short' })).toThrow()
  })

  it('rejects recommendation shorter than 10 chars', () => {
    expect(() => reviewRuleSchema.parse({ ...validRule, recommendation: 'fix it' })).toThrow()
  })

  it('rejects empty id', () => {
    expect(() => reviewRuleSchema.parse({ ...validRule, id: '' })).toThrow()
  })

  it('rejects empty name', () => {
    expect(() => reviewRuleSchema.parse({ ...validRule, name: '' })).toThrow()
  })

  it('rejects invalid severity', () => {
    expect(() => reviewRuleSchema.parse({ ...validRule, severity: 'high' })).toThrow()
  })

  it('rejects invalid scope', () => {
    expect(() => reviewRuleSchema.parse({ ...validRule, scope: 'global' })).toThrow()
  })
})
...
describe('triggerConditionSchema', () => {
  ...
  it('rejects domain trigger without domains', () => {
    expect(() => triggerConditionSchema.parse({ type: 'domain' })).toThrow()
  })

  it('rejects domain trigger with empty domains', () => {
    expect(() => triggerConditionSchema.parse({ type: 'domain', domains: [] })).toThrow()
  })

  it('rejects risk trigger without riskLevel', () => {
    expect(() => triggerConditionSchema.parse({ type: 'risk' })).toThrow()
  })

  it('rejects content trigger without pattern', () => {
    expect(() => triggerConditionSchema.parse({ type: 'content' })).toThrow()
  })

  it('rejects invalid trigger type', () => {
    expect(() => triggerConditionSchema.parse({ type: 'manual' })).toThrow()
  })
  ...
  it('rejects risk trigger with severity values', () => {
    expect(() => triggerConditionSchema.parse({ type: 'risk', riskLevel: 'major' })).toThrow()
    expect(() => triggerConditionSchema.parse({ type: 'risk', riskLevel: 'warning' })).toThrow()
  })
})
...
describe('compositionPolicySchema', () => {
  const validPolicy = {
    ...
    priority: 50,
    description: 'Always dispatch the build reviewer for code quality checks',
  }
  ...
  it('rejects priority below 0', () => {
    expect(() => compositionPolicySchema.parse({ ...validPolicy, priority: -1 })).toThrow()
  })

  it('rejects priority above 100', () => {
    expect(() => compositionPolicySchema.parse({ ...validPolicy, priority: 101 })).toThrow()
  })
  ...
  it('rejects empty dispatch', () => {
    expect(() => compositionPolicySchema.parse({ ...validPolicy, dispatch: '' })).toThrow()
  })
})
...
describe('agentRegistryEntrySchema', () => {
  const validAgent = {
    ...
    description: 'Reviews code for quality, DRY violations, type safety, and project conventions',
  ...
  it('rejects empty tools array', () => {
    expect(() => agentRegistryEntrySchema.parse({ ...validAgent, tools: [] })).toThrow()
  })

  it('rejects empty capabilities array', () => {
    expect(() => agentRegistryEntrySchema.parse({ ...validAgent, capabilities: [] })).toThrow()
  })
})
...
describe('domainMappingSchema', () => {
  const validMapping = {
    ...
    domain: 'financial',
    description: 'Financial arithmetic helpers using big.js for precision',
  }
  ...
  it('rejects empty pattern', () => {
    expect(() => domainMappingSchema.parse({ ...validMapping, pattern: '' })).toThrow()
  })

  it('rejects empty domain', () => {
    expect(() => domainMappingSchema.parse({ ...validMapping, domain: '' })).toThrow()
  })
