...
describe('scratchNoteSchema', () => {
  ...
  it('rejects empty content', () => {
    expect(() => scratchNoteSchema.parse({ ...validNote, content: '' })).toThrow()
  })

  it('rejects invalid UUID', () => {
    expect(() => scratchNoteSchema.parse({ ...validNote, id: 'bad-id' })).toThrow()
  })

  it('rejects invalid datetime', () => {
    expect(() => scratchNoteSchema.parse({ ...validNote, createdAt: 'not-a-date' })).toThrow()
  })
})
