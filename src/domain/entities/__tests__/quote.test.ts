...
describe('quoteStatusEnum', () => {
  it.each(['draft', 'sent', 'accepted', 'declined', 'revised'])("accepts '%s'", (status) => {
...
describe('serviceTypeEnum', () => {
  it.each(['screen-print', 'dtf', 'embroidery'])("accepts '%s'", (type) => {
...
describe('printLocationDetailSchema', () => {
  ...
  it('rejects empty location', () => {
    expect(() => printLocationDetailSchema.parse({ ...validDetail, location: '' })).toThrow()
  })

  it('rejects zero colorCount', () => {
    expect(() => printLocationDetailSchema.parse({ ...validDetail, colorCount: 0 })).toThrow()
  })

  it('rejects negative setupFee', () => {
    expect(() => printLocationDetailSchema.parse({ ...validDetail, setupFee: -1 })).toThrow()
  })
...
describe('discountSchema', () => {
  ...
  it.each(['manual', 'contract', 'volume'] as const)("accepts type '%s'", (type) => {
    expect(discountSchema.parse({ ...validDiscount, type }).type).toBe(type)
  })

  it('rejects empty label', () => {
    expect(() => discountSchema.parse({ ...validDiscount, label: '' })).toThrow()
  })

  it('rejects zero amount', () => {
    expect(() => discountSchema.parse({ ...validDiscount, amount: 0 })).toThrow()
  })

  it('rejects negative amount', () => {
    expect(() => discountSchema.parse({ ...validDiscount, amount: -50 })).toThrow()
  })

  it('rejects invalid type', () => {
    expect(() => discountSchema.parse({ ...validDiscount, type: 'loyalty' })).toThrow()
  })
})
...
describe('quoteLineItemSchema', () => {
  ...
  it('rejects negative unit price', () => {
    expect(() => quoteLineItemSchema.parse({ ...validItem, unitPrice: -1 })).toThrow()
  })
  ...
  it('rejects negative lineTotal', () => {
    expect(() => quoteLineItemSchema.parse({ ...validItem, lineTotal: -1 })).toThrow()
  })
  ...
  it('rejects negative size quantities', () => {
    expect(() => quoteLineItemSchema.parse({ ...validItem, sizes: { M: -1 } })).toThrow()
  })
...
describe('quoteSchema', () => {
    ...
    lineItems: [
      {
        ...
        sizes: { S: 5, M: 15, L: 20, XL: 10 },
        serviceType: 'screen-print',
        printLocationDetails: [{ location: 'Front', colorCount: 3, setupFee: 75 }],
        unitPrice: 14.5,
        lineTotal: 725,
      },
    ],
  ...
  it('rejects negative setup fees', () => {
    expect(() => quoteSchema.parse({ ...validQuote, setupFees: -50 })).toThrow()
  })

  it('rejects negative subtotal', () => {
    expect(() => quoteSchema.parse({ ...validQuote, subtotal: -1 })).toThrow()
  })

  it('rejects negative total', () => {
    expect(() => quoteSchema.parse({ ...validQuote, total: -1 })).toThrow()
  })

  it('rejects negative shipping', () => {
    expect(() => quoteSchema.parse({ ...validQuote, shipping: -10 })).toThrow()
  })

  it('rejects negative tax', () => {
    expect(() => quoteSchema.parse({ ...validQuote, tax: -5 })).toThrow()
  })

  it('rejects invalid datetime', () => {
    expect(() => quoteSchema.parse({ ...validQuote, createdAt: 'not-a-date' })).toThrow()
  })

  it('rejects invalid updatedAt datetime', () => {
    expect(() => quoteSchema.parse({ ...validQuote, updatedAt: 'not-a-date' })).toThrow()
  })
