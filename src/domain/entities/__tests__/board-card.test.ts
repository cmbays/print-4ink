...
describe('scratchNoteCardSchema', () => {
  ...
  it("enforces lane is always 'ready'", () => {
    expect(() => scratchNoteCardSchema.parse({ ...validCard, lane: 'in_progress' })).toThrow()
  })

  it('rejects empty content', () => {
    expect(() => scratchNoteCardSchema.parse({ ...validCard, content: '' })).toThrow()
  })

  it('rejects content over 500 chars', () => {
    expect(() => scratchNoteCardSchema.parse({ ...validCard, content: 'x'.repeat(501) })).toThrow()
  })
})
...
describe('quoteCardSchema', () => {
  ...
  it('rejects invalid quoteStatus', () => {
    expect(() => quoteCardSchema.parse({ ...validCard, quoteStatus: 'approved' })).toThrow()
  })

  it('rejects invalid lane', () => {
    expect(() => quoteCardSchema.parse({ ...validCard, lane: 'pending' })).toThrow()
  })

  it('rejects empty customerName', () => {
    expect(() => quoteCardSchema.parse({ ...validCard, customerName: '' })).toThrow()
  })
...
describe('jobCardSchema', () => {
  ...
  it('rejects invalid invoiceStatus', () => {
    expect(() => jobCardSchema.parse({ ...validCard, invoiceStatus: 'overdue' })).toThrow()
  })

  it('rejects zero quantity', () => {
    expect(() => jobCardSchema.parse({ ...validCard, quantity: 0 })).toThrow()
  })

  it('rejects invalid risk level', () => {
    expect(() => jobCardSchema.parse({ ...validCard, riskLevel: 'critical' })).toThrow()
  })

  it('rejects assigneeInitials over 3 chars', () => {
    expect(() => jobCardSchema.parse({ ...validCard, assigneeInitials: 'ABCD' })).toThrow()
  })
...
describe('boardCardSchema (discriminated union)', () => {
  ...
  it('rejects invalid type discriminator', () => {
    expect(() => boardCardSchema.parse({ type: 'invoice', id: 'test' })).toThrow()
  })
})
