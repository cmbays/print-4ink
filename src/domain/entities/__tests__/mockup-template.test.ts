...
describe('printZoneSchema', () => {
  ...
  it('rejects zero width (degenerate zone)', () => {
    expect(() => printZoneSchema.parse({ ...validZone, width: 0 })).toThrow()
  })

  it('rejects zero height (degenerate zone)', () => {
    expect(() => printZoneSchema.parse({ ...validZone, height: 0 })).toThrow()
  })
  ...
  it('rejects zone that overflows horizontally (x + width > 100)', () => {
    expect(() => printZoneSchema.parse({ ...validZone, x: 80, width: 30 })).toThrow()
  })

  it('rejects zone that overflows vertically (y + height > 100)', () => {
    expect(() => printZoneSchema.parse({ ...validZone, y: 85, height: 20 })).toThrow()
  })
})
...
describe('mockupTemplateSchema', () => {
  ...
  it('rejects negative viewBoxHeight', () => {
    expect(() => mockupTemplateSchema.parse({ ...validTemplate, viewBoxHeight: -10 })).toThrow()
  })
})
