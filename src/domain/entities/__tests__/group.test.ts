...
describe('groupSchema', () => {
  ...
  it('rejects empty name', () => {
    expect(() => groupSchema.parse({ ...validGroup, name: '' })).toThrow()
  })

  it('rejects invalid UUID for customerId', () => {
    expect(() => groupSchema.parse({ ...validGroup, customerId: 'not-a-uuid' })).toThrow()
  })

  it('rejects invalid UUID for id', () => {
    expect(() => groupSchema.parse({ ...validGroup, id: 'not-a-uuid' })).toThrow()
  })
})
