...
describe("artworkSchema", () => {
  ...
  it("rejects empty name", () => {
    expect(() =>
      artworkSchema.parse({ ...validArtwork, name: "" })
    ).toThrow();
  });

  it("rejects zero colorCount", () => {
    expect(() =>
      artworkSchema.parse({ ...validArtwork, colorCount: 0 })
    ).toThrow();
  });

  it("rejects negative colorCount", () => {
    expect(() =>
      artworkSchema.parse({ ...validArtwork, colorCount: -1 })
    ).toThrow();
  });

  it("rejects invalid customerId", () => {
    expect(() =>
      artworkSchema.parse({ ...validArtwork, customerId: "not-uuid" })
    ).toThrow();
  });
  ...
  it("rejects invalid datetime", () => {
    expect(() =>
      artworkSchema.parse({ ...validArtwork, createdAt: "not-a-date" })
    ).toThrow();
  });

  it("rejects zero width", () => {
    expect(() =>
      artworkSchema.parse({ ...validArtwork, width: 0 })
    ).toThrow();
  });
});
