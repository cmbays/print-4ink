...
function computeStats(invoices: Invoice[]) {
  ...
  const totalOutstanding = toNumber(
    outstandingInvoices.reduce((sum, inv) => sum.plus(money(inv.balanceDue)), money(0))
  )
  ...
  const overdueTotal = toNumber(
    overdueInvoices.reduce((sum, inv) => sum.plus(money(inv.balanceDue)), money(0))
  )

  // 3. Paid This Month — invoices with paidAt in current month
  const now = new Date()
  const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1)
  const paidThisMonth = invoices.filter((inv) => inv.paidAt && new Date(inv.paidAt) >= startOfMonth)
...
export function InvoiceStatsBar({ invoices }: InvoiceStatsBarProps) {
  const { totalOutstanding, overdueCount, overdueTotal, paidThisMonth, avgDays } =
  ...
  const values: Record<(typeof stats)[number]['key'], ReactNode> = {
    ...
    overdue: (
      <>
        {overdueCount} (<MoneyAmount value={overdueTotal} format="compact" />)
      </>
    ),
