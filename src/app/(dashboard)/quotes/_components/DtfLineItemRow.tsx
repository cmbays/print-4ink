...
export function DtfLineItemRow({ item, onUpdate, onRemove, canRemove }: DtfLineItemRowProps) {
  ...
  return (
    <div className={cn('rounded-lg border border-border bg-elevated p-3 space-y-3')}>
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        {item.sizePreset === 'custom' && (
          <>
            <div className="space-y-1">
              <label htmlFor={`width-${item.id}`} className="text-xs text-muted-foreground">
                Width (in)
              </label>
              <Input
                id={`width-${item.id}`}
            ...
            <div className="space-y-1">
              <label htmlFor={`height-${item.id}`} className="text-xs text-muted-foreground">
                Height (in)
              </label>
              <Input
                id={`height-${item.id}`}
        ...
        <div className="space-y-1">
          ...
          <Input
            ...
            value={item.quantity}
            onChange={(e) =>
              onUpdate(item.id, 'quantity', Math.max(1, parseInt(e.target.value, 10) || 1))
            }
