      ...
      {STEP_LABELS.map((label, i) => {
        ...
        return (
          <div key={label} className="flex items-center gap-1">
            {i > 0 && <div className={cn('h-px w-6', isCompleted ? 'bg-action' : 'bg-border')} />}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          <Layers
            className={cn('size-8', value === 'dtf' ? 'text-action' : 'text-muted-foreground')}
          />
          <div className="text-center">
            <p className="text-sm font-medium">DTF Transfer</p>
            <p className="text-xs text-muted-foreground mt-1">Sheet sizes, film types, rush fees</p>
          </div>
        </button>
      </div>
    </div>
    ...
    <div className="space-y-5">
      <div className="space-y-2">
        ...
        <Input
          id="template-name"
          placeholder={
            serviceType === 'screen-print' ? 'e.g., Standard Screen Print' : 'e.g., DTF Retail'
          }
          value={name}
      ...
      <div className="flex items-center gap-3">
        <Checkbox
          ...
          onCheckedChange={(checked) => onUseIndustryTemplateChange(checked === true)}
        />
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            ...
            {serviceType === 'screen-print' ? 'standard screen print' : 'retail DTF'} industry
            defaults
          </p>
        </div>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-border">
        ...
        {quantityTiers.map((tier, idx) => (
          <div key={idx} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-px bg-border">
...
function DTFPricingStep({
  ...
  const addTier = () => {
    ...
    onSheetTiersChange([...sheetTiers, { width: 22, length: newLength, retailPrice: 0 }])
  }
    ...
    <div className="space-y-6 max-h-[400px] overflow-y-auto pr-1">
      ...
      <div className="space-y-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Percentage markup over t-shirt base price</p>
        </div>
        <div className="grid gap-2">
        ...
        <div className="grid gap-2">
          {filmTypes.map((ft, idx) => (
            <div key={ft.type} className="flex items-center gap-3">
              <span className="text-xs w-24 text-muted-foreground">{filmLabels[ft.type]}</span>
              <div className="relative w-24">
                <Input
                  type="number"
                  min={0.1}
...
function SPPreviewStep({ template }: { template: PricingTemplate }) {
  const [sampleQty, setSampleQty] = useState(48)
  const [sampleColors, setSampleColors] = useState(3)

  const matrixData = useMemo(() => buildFullMatrixData(template, DEFAULT_GARMENT_COST), [template])

  const sampleCalc = useMemo(() => {
    return calculateScreenPrintPrice(sampleQty, sampleColors, ['front'], 't-shirts', template)
            ...
            <tbody>
              {matrixData.map((row) => (
                <tr key={row.tierLabel} className="border-t border-border">
                  <td className="px-2 py-1.5 font-medium text-muted-foreground">{row.tierLabel}</td>
        ...
        <div className="flex items-end gap-4">
          ...
          <div className="flex items-center gap-3 rounded-lg bg-card px-4 py-2">
            <div>
              ...
              <p className="text-sm font-semibold">{formatCurrency(sampleCalc.pricePerPiece)}</p>
            </div>
            <div>
...
function DTFPreviewStep({ template }: { template: DTFPricingTemplate }) {
  const [sampleLength, setSampleLength] = useState(template.sheetTiers[0]?.length ?? 24)
  ...
  const sampleCalc = useMemo(() => {
    return calculateDTFPrice(sampleLength, 'standard', 'standard', 'standard', template)
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {tierMargins.map(({ tier, margin }) => (
                <tr key={tier.length} className="border-t border-border">
                  <td className="px-3 py-1.5 font-medium text-muted-foreground">
                    22&quot; x {tier.length}&quot;
                  </td>
                  <td className="px-3 py-1.5 text-right">{formatCurrency(tier.retailPrice)}</td>
        ...
        <div className="flex items-end gap-4">
          ...
          <div className="flex items-center gap-3 rounded-lg bg-card px-4 py-2">
            <div>
              ...
              <p className="text-sm font-semibold">{formatCurrency(sampleCalc.price)}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Margin</p>
              <div className="flex items-center gap-1">
...
export function SetupWizard({ open, onOpenChange, onSave }: SetupWizardProps) {
  ...
  const stepInfo: Record<number, { title: string; description: string }> = {
    ...
    3: {
      title: serviceType === 'screen-print' ? 'Quantity Tiers & Pricing' : 'Sheet Size Pricing',
      description:
        serviceType === 'screen-print'
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex-1 py-2">
          {/* Step 1 */}
          {step === 1 && <ServiceTypeStep value={serviceType} onChange={setServiceType} />}
          ...
          {step === 3 && serviceType === 'screen-print' && (
            <SPPricingStep
              ...
              onTiersChange={(tiers) => setSpMatrix((m) => ({ ...m, quantityTiers: tiers }))}
          ...
          {step === 3 && serviceType === 'dtf' && (
            <DTFPricingStep sheetTiers={dtfSheetTiers} onSheetTiersChange={setDtfSheetTiers} />
          )}

          {/* Step 4 */}
          {step === 4 && serviceType === 'screen-print' && (
            <SPSettingsStep matrix={spMatrix} onMatrixChange={setSpMatrix} />
          )}
          ...
          {step === 5 && serviceType === 'dtf' && <DTFPreviewStep template={previewDTFTemplate} />}
        </div>
