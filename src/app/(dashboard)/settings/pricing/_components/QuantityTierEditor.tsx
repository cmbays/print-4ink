...
export function QuantityTierEditor({ tiers, basePrices, onTiersChange }: QuantityTierEditorProps) {
        ...
        return (
          <div key={index} className="space-y-1">
            ...
            <div className="flex items-center gap-3 pl-0.5">
              <span className="text-[10px] text-muted-foreground tabular-nums">{tier.label}</span>
              {minError && minMessage && (
            ...
            {gap && (
              <div className="flex items-center gap-1.5 rounded bg-error/5 border border-error/20 px-2 py-1 text-[11px] text-error">
                ...
                {gap.missingStart !== gap.missingEnd && `\u2013${gap.missingEnd}`} not covered
              </div>
            )}
          </div>
        )
