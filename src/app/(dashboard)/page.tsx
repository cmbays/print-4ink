...
import { AlertTriangle, Calendar, Clock, CheckCircle2, Package, Kanban } from 'lucide-react'
...
export default async function DashboardPage() {
  ...
  const inProgressJobs = jobs.filter((j) => j.lane === 'in_progress' || j.lane === 'review')
  ...
  const capacitySummary = {
    ...
    rushQuantity: jobs
      .filter((j) => j.lane !== 'done' && j.priority === 'rush')
            ...
            {inProgressJobs.map((job) => (
              ...
              >
                <div className="min-w-0">
                  <p className="text-sm font-medium">{job.title}</p>
                  <p className="text-xs text-muted-foreground">
                    {job.jobNumber} &middot; {getCustomerName(job.customerId)} &middot; Due{' '}
                    {job.dueDate}
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant="outline">{LANE_LABELS[job.lane]}</Badge>
                  <Badge variant="secondary">{PRIORITY_LABELS[job.priority]}</Badge>
                </div>
              </Link>
            ))}
          </CardContent>
                ...
                {comingUpJobs.slice(0, 5).map((job) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {job.jobNumber} &middot; Due {new Date(job.dueDate).toLocaleDateString()}
                      </p>
                    </div>
