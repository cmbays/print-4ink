...
export function RiskIndicator({ riskLevel, showLabel = false, className }: RiskIndicatorProps) {
  ...
  return (
    <span className={cn('inline-flex items-center gap-1.5', className)} aria-label={label}>
      ...
      />
      {showLabel && <span className={cn('text-xs font-medium', colorClass)}>{label}</span>}
    </span>
  )
}
