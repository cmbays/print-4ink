...
describe('classify', () => {
  it('classifies a small feature PR touching financial domain', () => {
    const facts = makeFacts({
      files: [
        ...
        {
          path: 'components/features/Quote/QuoteCalc.tsx',
          additions: 20,
          deletions: 3,
          status: 'modified',
        },
  ...
  it('classifies a large refactor PR as high risk', () => {
    const facts = makeFacts({
      ...
      commits: [
        {
          sha: 'def456',
          message: 'refactor: restructure review pipeline schemas',
          author: 'Claude',
        },
      ],
    })
  ...
  it('classifies a docs-only PR as low risk', () => {
    const facts = makeFacts({
      files: [{ path: 'docs/PRD.md', additions: 5, deletions: 1, status: 'modified' }],
      totalAdditions: 5,
      totalDeletions: 1,
  ...
  it('classifies a test-only PR correctly', () => {
    const facts = makeFacts({
      files: [
        {
          path: 'lib/review/__tests__/load-config.test.ts',
          additions: 50,
          deletions: 10,
          status: 'modified',
        },
        {
          path: 'lib/schemas/__tests__/review-config-data.test.ts',
          additions: 30,
          deletions: 5,
          status: 'added',
        },
      ...
      totalDeletions: 15,
      commits: [
        { sha: 'jkl012', message: 'test: add review config validation tests', author: 'Claude' },
      ],
    })
  ...
  it('classifies mixed PR with multiple domains', () => {
    const facts = makeFacts({
      files: [
        ...
        {
          path: 'lib/review/__tests__/classify.test.ts',
          additions: 40,
          deletions: 0,
          status: 'added',
        },
