...
export async function runReviewOrchestration(
  ...
  try {
    ...
    amendedManifest = z.array(agentManifestEntrySchema).parse(gapResult.manifest)
    ...
    gaps = [
      {
        concern: 'Gap analyzer failed — pipeline continued with Stage 3 manifest',
        recommendation: 'Investigate gap analyzer failure and re-run review',
        confidence: 0,
      },
    ]
  }
