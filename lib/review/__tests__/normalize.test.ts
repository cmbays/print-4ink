...
describe('normalize (Stage 1 — git diff to PRFacts)', () => {
  ...
  it('parses a simple diff with added and modified files', () => {
    setupGitMocks({
      numstat: ['10\t2\tlib/review/normalize.ts', '5\t3\tlib/schemas/review-pipeline.ts'].join(
        '\n'
      ),
      nameStatus: ['A\tlib/review/normalize.ts', 'M\tlib/schemas/review-pipeline.ts'].join('\n'),
      log: [
    ...
    const addedFile = parsed.files.find((f) => f.path === 'lib/review/normalize.ts')
    ...
    const modifiedFile = parsed.files.find((f) => f.path === 'lib/schemas/review-pipeline.ts')
  ...
  it('handles binary files (- in numstat) as 0 additions/deletions', () => {
    setupGitMocks({
      numstat: ['-\t-\tpublic/logo.png', '3\t1\tREADME.md'].join('\n'),
      nameStatus: ['A\tpublic/logo.png', 'M\tREADME.md'].join('\n'),
  ...
  it('handles renamed files from --name-status -M', () => {
    setupGitMocks({
      numstat: ['2\t1\tlib/{old-name.ts => new-name.ts}'].join('\n'),
      nameStatus: ['R100\tlib/old-name.ts\tlib/new-name.ts'].join('\n'),
  ...
  it('handles deleted files', () => {
    setupGitMocks({
      numstat: ['0\t50\tlib/deprecated.ts', '0\t30\tlib/old-utils.ts'].join('\n'),
      nameStatus: ['D\tlib/deprecated.ts', 'D\tlib/old-utils.ts'].join('\n'),
