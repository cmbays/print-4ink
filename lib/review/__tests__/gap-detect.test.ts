...
const baseFacts: PRFacts = {
  ...
  totalDeletions: 2,
  commits: [
    { sha: "abc123", message: "feat: update money helper", author: "Claude" },
  ],
};
...
describe("gapDetect (Stage 4 — Gap Detect)", () => {
  ...
  it("amends manifest when analyzer returns additional agents", async () => {
    ...
    const securityAgent = parsed.manifest.find(
      (a) => a.agentId === "security-reviewer",
    );
    ...
    expect(parsed.gaps[0].concern).toBe(
      "Financial helper changes may expose injection vectors",
    );
    expect(parsed.gaps[0].suggestedAgent).toBe("security-reviewer");

    // Original agents should still be present
    expect(
      parsed.manifest.find((a) => a.agentId === "build-reviewer"),
    ).toBeDefined();
    expect(
      parsed.manifest.find((a) => a.agentId === "finance-sme"),
    ).toBeDefined();
  });
  ...
  it("does not duplicate agents already in manifest — merges scope, rules, and priority", async () => {
    ...
    expect(merged!.reason).toContain(
      "Gap analysis found tax calculation concern",
    );
