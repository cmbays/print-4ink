...
describe("brandPreferenceSchema", () => {
  ...
  it("rejects empty brand name", () => {
    expect(() =>
      brandPreferenceSchema.parse({ ...validBrandPref, brandName: "" })
    ).toThrow();
  });

  it("rejects invalid inheritance mode", () => {
    expect(() =>
      brandPreferenceSchema.parse({ ...validBrandPref, inheritMode: "merge" })
    ).toThrow();
  });
...
describe("customerPreferenceSchema", () => {
  ...
  it("rejects missing fields", () => {
    expect(() =>
      customerPreferenceSchema.parse({ inheritMode: "inherit" })
    ).toThrow();
  });
