...
describe("screenSchema", () => {
  ...
  it("rejects zero mesh count", () => {
    expect(() =>
      screenSchema.parse({ ...validScreen, meshCount: 0 })
    ).toThrow();
  });

  it("rejects negative mesh count", () => {
    expect(() =>
      screenSchema.parse({ ...validScreen, meshCount: -110 })
    ).toThrow();
  });

  it("rejects fractional mesh count", () => {
    expect(() =>
      screenSchema.parse({ ...validScreen, meshCount: 160.5 })
    ).toThrow();
  });

  it("rejects empty emulsion type", () => {
    expect(() =>
      screenSchema.parse({ ...validScreen, emulsionType: "" })
    ).toThrow();
  });

  it("rejects invalid UUID for jobId", () => {
    expect(() =>
      screenSchema.parse({ ...validScreen, jobId: "not-uuid" })
    ).toThrow();
  });
});
