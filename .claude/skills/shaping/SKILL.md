...
1. **Frame** — ground truth for source material, problem statement, outcome definition
...
2. **Ask: "Does this affect the other document?"**
...
- **Breadboard** - Use the `/breadboarding` skill to map the system into concrete affordances
...
Frame (problem/outcome)
    |
    v
Shaping (explore, detail, select shape)
    |
    v
[handoff to /breadboarding]
```
...
**Shaping doc** is where active work happens. All exploration, requirements gathering, shape comparison, and fit checking happens here. This is the working document and ground truth for R, shapes, parts, and fit checks.
...
Every shaping document (shaping doc, frame) must include `shaping: true` in its YAML frontmatter. This enables tooling hooks (e.g., ripple-check reminders) that help maintain consistency across documents.
...
See **Multi-Level Consistency** at the top of this document. Changes to the Frame or Shaping doc must ripple to the other document if affected. The system only works if both levels are consistent with each other.
